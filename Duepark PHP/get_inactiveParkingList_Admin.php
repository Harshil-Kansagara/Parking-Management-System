<?php
    ...
    if($employeeRole == "Manager" || $employeeId == "Sale"){
        ...
        else
        {
            if(mysqli_num_rows($getAllParking_result)>0){
                while($row=mysqli_fetch_assoc($getAllParking_result)){
                    ...
                    $sql_query = "select id, ParkingId, ParkingAcronym, ParkingName, ParkingActiveState from ".$parkingTable." where ParkingActiveState=false and id = $parkingId limit 1";
    ...
    {
        $sql_query = "select id, ParkingId, ParkingAcronym, ParkingName, ParkingActiveState from ".$parkingTable." where ParkingActiveState=false";
        $sql_result = mysqli_query($connection, $sql_query);
